import { Container,Button,Row, Text,Col, Spacer,Input,Radio, Popover,Grid} from "@nextui-org/react";
...
import { FormatDateInitial } from "../../../data/formatData";
  ...
  const del = useQuery(["delete", document], () => deletePerson(document), {
    ...
    const onSubmit = (data) => {
      ...
      delete data.birthDate;
        ...
        const people ={
          ...newData,nameUser: user.nameUser,birthDate:utcDate,
        };
        console.log(people);
        query.mutate(people);
    ...
    const {data, isLoading} = useQuery(["search",user], ()=> getPerson(user.personDocument,requestOptions),{ enabled: !!user,refetchOnWindowFocus:false,retry:false});
    ...
    const departments = useQuery("departaments",() => getAllDepartments(requestOptions),{refetchOnWindowFocus:false,retry:false});
    
    const cities = useQuery(["cities",city],() => getCities(city,requestOptions),{enabled: !!city,refetchOnWindowFocus:false,retry:false});
    ...
    return(
        <Container id="containerPrincipal" css={{paddingTop:'10px',height:'40rem', overflow:'hidden'}} >
            ...
            >
              <Row justify="center" align="center">
                ...
                <Spacer x={1}/>
                <Button size="sm" shadow color="error" onPress={handleDeleteClick}>
                  Eliminar
                </Button>
            </Grid.Container>
            </Popover.Content>
            ...
            <Container id="containerForm" css={{paddingTop:'10px',height:'27.5rem',overflowY:'auto'}}  >
               <form onSubmit={handleSubmit(onSubmit)}>
                <Row justify="flex-start" >
                    ...
                    <Col css={{width:'80%'}}>
                        ...
                        color="secondary"
                        >
                        <Radio value="TI" size="sm">TI</Radio>
                        ...
                        )} label="Documento"  width="15rem" clearable />
                        ...
                        <Row  justify="flex-start" align="center" css={{height:'4rem'}}>
                          ...
                          <Input {...register("firstName",
                          {
                            ...
                            }})} label="Primer Nombre" width="15rem" clearable />
                          <Text css={{fontSize:'0.85rem'}}>{errors.firstName?.message}</Text>
                          {errors.firstName && errors.firstName.type === "pattern" && (
                            <Text css={{fontSize:'0.85rem'}}>Solo se permiten letras y espacios</Text>
                          )}
                          </Col>
                          <Spacer x={2}/>
                          <Col css={{height:'4rem', width:'15rem'}}>
                            <Input {...register("secondName",{
                              ...
                              value: 20,
                              message: 'Max 20 caracteres' 
                            }})} label="Segundo Nombre" width="15rem" clearable />
                            <Text css={{fontSize:'0.85rem'}}>{errors.secondName?.message}</Text>
                        ...
                        <Row  justify="flex-start" align="center" css={{height:'4rem'}}>
                          ...
                          <Input {...register("firstLastName",
                          {
                            ...
                            maxLength: {
                              value: 20,
                              message: 'Max 20 caracteres' 
                            }})} label="Primer Apellido" width="15rem" clearable />
                          <Text css={{fontSize:'0.85rem'}}>{errors.firstLastName?.message}</Text>
                          ...
                          <Col css={{height:'4rem', width:'15rem'}}>
                            <Input {...register("secondLastName",{
                              ...
                              value: 20,
                              message: 'Max 20 caracteres' 
                            }})} label="Segundo Apellido" width="15rem" clearable />
                            <Text css={{fontSize:'0.85rem'}}>{errors.secondLastName?.message}</Text>
                        ...
                        <Radio id="sexo" value="male" size="sm">Masculino</Radio>
                        </Radio.Group>
                        ...
                        <Row justify="flex-start" align="center" css={{height:'4rem'}}>
                            <Col css={{height:'4rem', width:'15rem'}}>
                              <Input {...register("phone",{ 
                                ...
                                }})} clearable label="Telefono" width="15rem" />
                              <Text css={{fontSize:'0.85rem'}}>{errors.phone?.message}</Text>
                            ...
                            <Spacer x={2}/>
                            <Col css={{height:'4rem', width:'15rem'}}>
                              <Input {...register("institutionalMail",{ 
                                required: true })} clearable label="Correo" type="email" width="15rem" />
                              <Text css={{fontSize:'0.85rem'}}>{errors.institutionalMail?.message}</Text>
                            </Col>
                        </Row>
                        <Spacer y={1.7}/>
                        <Row justify="flex-start" align="center" css={{height:'4rem'}}>
                        ...
                        <Select {...register("departments",{ required: true })}>
                            ...
                            ? ( departments.data.data.map((p)=> ( <option  key={p.name} value={p.name}>{p.name}</option>))
                            ) : "" }
                        </Select>
                        </Col>
                        <Spacer x={2}/>
                        ...
                        <Select {...register("citiesCode",{ required: true })}>
                            ...
                            ? ( cities.data.data.map((p)=> ( <option key={p.id} value={p.id}>{p.name}</option>))
                            ) : "" }
                        </Select>
                        </Col>
                        </Row>
            ...
            {query.isSuccess  ? <Message type={"success"} title={"¡Formulario enviado correctamente!"} /> : query.isError ? <Message type={"error"} title={"¡Error al guardar el formulario!"}/>: "" }
            {del.isSuccess  ? <Message  type={"success"} title={"Formulario eliminado correctamente"} message={del.data.message} /> :del.isError ? <Message type={"error"} title={"¡Error al eliminar el formulario!"}/>: ""}
        </Container>
    );
