...
export function RegisterStudent() {
  ...
  const user = useMutation((user) => {
    return apiClient
      .post("students", user, requestOptions)
      .then((res) => res.data);
  });

  const program = useQuery("program", () => getAllProgram(), {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <Col css={{ paddingTop: "10px", width: "40%" }}>
          ...
          <Col css={{ height: "7rem" }}>
            ...
            />
            <Text css={{ fontSize: "0.85rem" }}>
              {errors.amountCredits?.message}
            </Text>
            {errors.amountCredits &&
              errors.amountCredits.type === "pattern" && (
                <Text css={{ fontSize: "0.85rem" }}>
                  Solo se permiten numeros
                </Text>
              )}
      ...
      {user.isSuccess ? (
        <Message type={"success"} title={"Estudiante creado correctamente"} />
      ) : user.isError ? (
        <Message type={"error"} title={"¡Error al crear Estudiante!"} />
      ) : (
        ""
      )}
    </Container>
