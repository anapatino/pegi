import { Container,Input,Radio,Button,Spacer, Text, Col, Row } from "@nextui-org/react";
...
export function RegisterUser (){
    ...
    const user = useMutation(user =>{
        return apiClient.post("auth/sign-up",user).then();
    });
    ...
    return(
        ...
        <Row justify="space-between" align="center">
          ...
          <Col css={{paddingTop:'10px',width:'50%', overflowY:'auto'}}>
           <Col css={{height:'7rem'}}>
            <Input {...register("name",{
                ...
                }})} label="Correo" width="22rem" clearable />
           ...
           <Col css={{height:'7rem'}}>
             <Input {...register("password",{
                    ...
                    value: 30,
                    message: 'Max 30 caracteres' 
                }})} label="Contraseña" width="22rem" clearable/>
            <Text css={{fontSize:'0.85rem'}}>{errors.password?.message}</Text>
            </Col>
            <Spacer y={3}/>
            <Col css={{height:'7rem'}}>
            <Radio.Group
         ...
         {user.isSuccess  ? <Message type={"success"} title={"Usuario creado correctamente"} /> : user.isError ? <Message type={"error"} title={"¡Error al crear usuario!"}/>: "" }
        </Container>
    );
}
