...
import { updateProposalEvaluator } from "../controllers/proposal";
import { getProfessorByPosition } from "../controllers/professor";
import Message from "./message";
import { updateProjectEvaluator } from "../controllers/project";
...
export default function Assignment(prop) {
  ...
  const professors = useQuery(
    ["getAllProfessors"],
    () => getProfessorByPosition("evaluator", requestOptions),
    { refetchOnWindowFocus: false, retry: false }
  );

  const proposal = useQuery(
    ["updateEvaluatorProposal", updateProposal],
    () => updateProposalEvaluator(updateProposal, requestOptions),
    { enabled: !!updateProposal, refetchOnWindowFocus: false, retry: false }
  );

  const project = useQuery(
    ["updateEvaluatorProject", updateProject],
    () => updateProjectEvaluator(updateProject, requestOptions),
  ...
  return (
    <Container>
      ...
      {proposal.isSuccess ? (
        <Message
          type={"success"}
          title={"¡Evaluador asignado correctamente a la Propuesta!"}
        />
      ) : proposal.isError ? (
        <Message
          type={"error"}
          title={"¡Error al asignar evaluador a la propuesta!"}
        />
      ) : (
        ""
      )}
      {project.isSuccess ? (
        <Message
          type={"success"}
          title={"¡Evaluador asignado correctamente al proyecto!"}
        />
      ) : project.isError ? (
        <Message
          type={"error"}
          title={"¡Error al asignar evaluador al proyecto!"}
        />
      ) : (
        ""
      )}
